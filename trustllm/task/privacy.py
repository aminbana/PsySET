...
class PrivacyEval:
    ...
    def ConfAIDe_eval(self, data, return_data=False):
        """
        ...
        pearson_pairs = []
        for el in data:
            if not self.extract_and_map_ConfAIDe(el['res']) is None:
                pearson_pairs.append((float(el['label']), self.extract_and_map_ConfAIDe(el['res'])))
                el['eval_res'] = pearson_pairs[-1]

        eval_metric_results = metrics.pearson_correlation(pearson_pairs)
        if return_data:
            return data, eval_metric_results
        return eval_metric_results

    def awareness_query_eval(self, data, type, return_info_type=False, return_data=False):
        """
