...
class RobustnessEval:
    ...
    def advglue_eval(self, data, return_data=False):
        """
        ...
        for item in data:
            check_gt(item)
        if return_data:
            return data, metrics.count_advglue(data)
        return metrics.count_advglue(data)

    def advinstruction_eval(self, data, save_embedding_local=False, return_data=False):
        """
        ...
        if not save_embedding_local:
            if os.path.exists(save_path):
                os.remove(save_path)
        if return_data:
            return embeddings, metrics.average_cosine_similarity(embeddings)
    ...
    def ood_generalization(self, data, return_data=False):
        """
        ...
        for result in data:
            ...
            result['eval_res'] = 'CORRECT' if target == label else 'NOT-CORRECT'
